...
export const generateDiacritics = (code_points) => {
	...
	patt_split_outer	= new RegExp('((?:'+ escapeToPattern(folded_chars) + accent_pat + '*)+)','gui');
	...
	if( expand_before ){
		// size: 63807
		folded_chars.slice(0).sort((a,b) => a.length -b.length).forEach((latin)=>{

			var substrings	= allSubstrings(latin);
			var pattern = substrings.map((sub_pat)=>{

				sub_pat = Array.from(sub_pat).map((l)=>{
					if( diacritics.hasOwnProperty(l) ){
						return setToPattern(diacritics[l]);
					}
					return l;
				});
...
export const diacriticRegexPoints = (regex) => {
	...
	/*
	 * close but this still doesn't work for "3/4". "3/" won't match and will be added to the final
	let foldeda				= Array.from(asciifold(regex));
	let buffer				= '';
	let patt_match_whole	= new RegExp('^(?:'+ escapeToPattern(folded_chars) + accent_pat + '*)+$','ui');
	let buffer_matches		= false;
	let final				= [];

	const addBuffer = (buffer) =>{
		if( buffer_matches ){
			let combos = uniqueCombos(buffer);
	...
	console.log('parts',foldeda);

	for( let part of foldeda ){
		let new_buffer = buffer + part;

		if( new_buffer.search(patt_match_whole) == -1 ){
			console.log('buffer doesnt match',buffer);
			addBuffer(buffer);
			buffer = part;
		...
		buffer_matches = true;
		buffer = new_buffer;
		console.log('buffer works',buffer);
	}
	...
	*/
	...
	/*
	let regexa = Array.from(regex.normalize('NFC'));
	let prev_match;
	let split = [];
	let buffer = '';
	regexa = regexa.forEach((part) => {

		let new_buffer = buffer + asciifold(part);

		let test = part.search(patt_split_inner);

		part = asciifold(part);
		if( test === prev_match ){
			part = outer_split.pop() + part;
		}

		outer_split.push(part);
		prev_match = test;
	});
	*/
	...
	// 2) loop through folded parts
	let foldeda = Array.from(asciifold(regex));
	let final = [];
	let current_part = '';
	let outer_part_buffer = '';
	for( let part of foldeda ){
		let currenti = final.length;
		let outer_part = outer_split[currenti];
		current_part = current_part + part;

		if( current_part == outer_part ){
			final.push(current_part);
			outer_split.shift();
			current_part = '';
			continue;
		}

		if( diacritic_patterns[current_part] ){
			final.push(current_part);
			current_part = '';
			continue;
		}
	...
	*/
	...
	//let outer_split2		= split(regex,patt_split_outer);
	let outer_split		= split(regex,patt_all_chars);
	let final = [];
	for( let part of outer_split){

		let test = part.search(patt_split_outer);
		if( test == -1 ){
			final.push(part);
		}else{
